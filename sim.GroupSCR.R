...
sim.GroupSCR <- function(Np=10,lambda.P=6,lam0.P=0.2,sigma.P=0.50,lambda.I=0.2,K=10,X=X,buff=3){
  library(VGAM)
  ...
  D <- e2dist(s,X)
  lamd.P <- lam0.P*exp(-D*D/(2*sigma.P*sigma.P))
  J <- nrow(X)
  ...
  for(i in 1:Ni){
    for(j in 1:J){
      for(k in 1:K){
        y.I[i,j,k] <- rpois(1,lambda.I*y.P[groupID[i],j,k])
      }
    }
  }
