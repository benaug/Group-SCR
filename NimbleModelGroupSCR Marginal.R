NimModel <- nimbleCode({
  ...
  for(i in 1:M){
    ...
    lamd.P[i,1:J] <- GetVisitRate(s=s[i,1:2],X=X[1:J,1:2],lam0.P=lam0.P,sigma.P=sigma.P,J=J,z=z[i])
...
GetVisitRate <- nimbleFunction(
  run = function(s = double(1), X=double(2),lam0.P=double(0), sigma.P=double(0), J=double(0), z=double(0)){ 
    returnType(double(1))
    if(z==0){
      return(rep(0,J))
    }else{
